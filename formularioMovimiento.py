...
from config import FUENTE, COLOR_BG, COLOR_TEXTO, COLOR_MUTED
...
class FormularioMovimiento(ctk.CTkToplevel):
    ...
    def _construir(self):
        ...
        # Título
        ctk.CTkLabel(scroll, text="Nuevo movimiento",
                     font=(FUENTE, 18, "bold"), text_color=COLOR_TEXTO,
        ...
        self.btnIngreso = ctk.CTkButton(
            frameTipo, text="↓  Ingreso", font=(FUENTE, 13), height=38,
            fg_color="#1D9E75", hover_color="#178a63",
            command=lambda: self._setTipo("ingreso"))
        self.btnIngreso.grid(row=0, column=0, sticky="ew", padx=(0, 4))

        self.btnGasto = ctk.CTkButton(
            frameTipo, text="↑  Gasto", font=(FUENTE, 13), height=38,
            fg_color="#E8E6DF", text_color=COLOR_MUTED, hover_color="#d9d7d0",
            command=lambda: self._setTipo("gasto"))
        self.btnGasto.grid(row=0, column=1, sticky="ew", padx=(4, 0))

        # Subcuenta (cuenta + sitio)
                # Cuenta
        ctk.CTkLabel(scroll, text="Cuenta *", font=(FUENTE, 11),
                     text_color=COLOR_MUTED, anchor="w"
                     ).pack(fill="x", padx=20, pady=(0, 4))
        ...
        self.comboCuenta = ctk.CTkComboBox(
            scroll, values=list(self.cuentasMap.keys()),
            font=(FUENTE, 13), height=36, state="readonly",
            command=self._actualizarSitios)
        if cuentas:
            self.comboCuenta.set(cuentas[0]["nombre"])
        ...
        self.comboSitio = ctk.CTkComboBox(
            scroll, values=[], font=(FUENTE, 13),
            height=36, state="readonly")
        self.comboSitio.pack(fill="x", padx=20, pady=(0, 12))

        # Llenar sitios de la primera cuenta
        self._actualizarSitios(cuentas[0]["nombre"] if cuentas else "")

        # Monto
        ctk.CTkLabel(scroll, text="Monto *", font=(FUENTE, 11),
                     text_color=COLOR_MUTED, anchor="w"
                     ).pack(fill="x", padx=20, pady=(0, 4))
        ...
        ctk.CTkLabel(frameMonto, text="$", font=(FUENTE, 20, "bold"),
                     text_color=COLOR_MUTED, width=24
                     ).grid(row=0, column=0, padx=(0, 4))

        self.entryMonto = ctk.CTkEntry(
            frameMonto, font=(FUENTE, 20, "bold"),
            height=42, placeholder_text="0.00")
        self.entryMonto.grid(row=0, column=1, sticky="ew")

        # Descripción
        ctk.CTkLabel(scroll, text="Descripción *", font=(FUENTE, 11),
                     text_color=COLOR_MUTED, anchor="w"
                     ).pack(fill="x", padx=20, pady=(0, 4))
        ...
        ctk.CTkLabel(frameFechaRef, text="Fecha *", font=(FUENTE, 11),
                     text_color=COLOR_MUTED, anchor="w"
                     ).grid(row=0, column=0, sticky="w", pady=(0, 4))
        self.entryFecha = ctk.CTkEntry(
            frameFechaRef, font=(FUENTE, 13), height=36,
            placeholder_text="YYYY-MM-DD")
        self.entryFecha.insert(0, date.today().isoformat())
        self.entryFecha.grid(row=1, column=0, sticky="ew", padx=(0, 6))

        ctk.CTkLabel(frameFechaRef, text="Referencia", font=(FUENTE, 11),
                     text_color=COLOR_MUTED, anchor="w"
        ...
        # Categoría
        ctk.CTkLabel(scroll, text="Categoría", font=(FUENTE, 11),
                     text_color=COLOR_MUTED, anchor="w"
                     ).pack(fill="x", padx=20, pady=(0, 4))
        self.categoriasMap = {}
        ...
        # Notas
        ctk.CTkLabel(scroll, text="Notas", font=(FUENTE, 11),
                     text_color=COLOR_MUTED, anchor="w"
                     ).pack(fill="x", padx=20, pady=(0, 4))
        self.txtNotas = ctk.CTkTextbox(
            scroll, font=(FUENTE, 13), height=70, corner_radius=6)
        ...
        ctk.CTkButton(
            ...
            height=38, width=100, fg_color="transparent",
        ...
        ctk.CTkButton(
            frameBotones, text="Guardar movimiento",
            font=(FUENTE, 13, "bold"), height=38,
    ...
    def _guardar(self):
        ...
        subcuentaId = self.sitiosMap.get(sitioNombre)
        if not subcuentaId:
    ...
    def _actualizarSitios(self, cuenta_nombre):
        cuenta_id = self.cuentasMap.get(cuenta_nombre)
        if not cuenta_id:
            return
        subcuentas = db.obtener_subcuentas(cuenta_id=cuenta_id)
        self.sitiosMap = {sc["sitio_nombre"]: sc["id"] for sc in subcuentas}
        nombres = list(self.sitiosMap.keys())
        self.comboSitio.configure(values=nombres)
        self.comboSitio.set(nombres[0] if nombres else "")
